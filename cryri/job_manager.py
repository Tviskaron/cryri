    ...
    def submit_run(self, cfg: CryConfig) -> str:
        ...
        if cfg.container.uv.enabled:
            ...
            if user_cmd.startswith("uv run "):
                inner_cmd = f'cd {quoted_dir} && pip install uv && export UV_CACHE_DIR="{cache_dir}" && uv sync && {user_cmd}'
            else:
                inner_cmd = f'cd {quoted_dir} && pip install uv && export UV_CACHE_DIR="{cache_dir}" && uv sync && uv run {user_cmd}'
